...
export function ElementModal({ element, onClose }: ElementModalProps) {
    const parseElectronConfiguration = () => {
        return element.electronicConfiguration
            .split(' ')
            .map((orbital) => {
                const match = /^(\d)([spdf])(\d+)$/.exec(orbital);
    ...
    const valenceElectrons = parsedConfig.slice(-2).reduce((acc, o) => acc + o.count, 0);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white p-6 rounded-lg w-96 relative shadow-lg">
                ...
                <p className="text-sm mb-4">
                    The electron configuration shows how electrons are distributed across the atom's
                    shells and orbitals. Each part like{' '}
                    <code className="bg-gray-200 px-1 rounded">1s2</code> means:
                </p>
                ...
                <ul className="text-sm space-y-2 mb-4">
                    {parsedConfig.map(({ orbital, shell, subshell, count }) => (
                        <li key={orbital}>
                            <strong>
                                Shell {shell}, {subshell}-orbital:
                            </strong>{' '}
                            {count} electron{count !== 1 ? 's' : ''}
                        </li>
                    ))}
                </ul>
                ...
                <div className="text-sm border-t pt-3 mt-3">
                    <p>
                        <strong>Total Electrons:</strong> {element.atomicNumber}
                    </p>
                    <p>
