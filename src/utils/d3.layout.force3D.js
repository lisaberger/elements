...
(function () {
    if (!(d3 && d3.layout && d3.geom)) {
        throw (
            'force3D: Unable to inject d3.layout.force3D' +
    ...
    d3.geom.octree = function (points, x1, y1, z1, x2, y2, z2) {
            ...
            function insert(n, d, x, y, z, x1, y1, z1, x2, y2, z2) {
                ...
                if (n.leaf) {
                    var nx = n.x,
                        ...
                        nz = n.z;
                    if (nx != null) {
                        if (abs(nx - x) + abs(ny - y) + abs(nz - z) < 0.01) {
        ...
        octree.size = function (_) {
            if (!arguments.length)
                return x1 == null ? null : [x2 - x1, y2 - y1, z2 - z1];
            if (_ == null) x1 = y1 = z1 = x2 = y2 = z2 = null;
    ...
    function d3_geom_octreeVisit(f, node, x1, y1, z1, x2, y2, z2) {
        if (!f(node, x1, y1, z1, x2, y2, z2)) {
            var sx = (x1 + x2) * 0.5,
                ...
                children = node.nodes;
            if (children[0])
                d3_geom_octreeVisit(f, children[0], x1, y1, z1, sx, sy, sz);
            if (children[1])
                d3_geom_octreeVisit(f, children[1], sx, y1, z1, x2, sy, sz);
            if (children[2])
                d3_geom_octreeVisit(f, children[2], x1, sy, z1, sx, y2, sz);
            if (children[3])
                d3_geom_octreeVisit(f, children[3], sx, sy, z1, x2, y2, sz);
            if (children[4])
                d3_geom_octreeVisit(f, children[4], x1, y1, sz, sx, sy, z2);
            if (children[5])
                d3_geom_octreeVisit(f, children[5], sx, y1, sz, x2, sy, z2);
            if (children[6])
                d3_geom_octreeVisit(f, children[6], x1, sy, sz, sx, y2, z2);
            if (children[7])
                d3_geom_octreeVisit(f, children[7], sx, sy, sz, x2, y2, z2);
        }
    }
    function d3_geom_octreeFind(root, x, y, z, x0, y0, z0, x3, y3, z3) {
        ...
        (function find(node, x1, y1, z1, x2, y2, z2) {
            if (x1 > x3 || y1 > y3 || z1 > z3 || x2 < x0 || y2 < y0 || z2 < z0)
                return;
            if ((point = node.point)) {
                var point,
                    dx = x - node.x,
            ...
            var children = node.nodes,
                ...
                deep = z >= zm;
            for (
                var i = (deep << 2) | (below << 1) | right, j = i + 8;
                i < j;
                ++i
            ) {
    ...
    d3.layout.force3D = function () {
        ...
        force.tick = function () {
            ...
            if (charge) {
                d3_layout_force3dAccumulate(
                    (q = d3.geom.octree(nodes)),
                    alpha,
                    charges,
                );
                i = -1;
